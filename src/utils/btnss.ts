...
class ToBrot extends mytype implements methodsMvc {
    ...
    on(eventName: string, fn: Function): void {
        let arr: Array<Function> = Array.isArray(this.list[eventName]) ? this.list[eventName] : new Array<Function>();
        this.list[eventName] = [...arr, fn];
    }

    emit(eventName: string, ...args: any[]): any {
        if (!Array.isArray(this.list[eventName])) return '改事件还未订阅，请订阅后发布。'
